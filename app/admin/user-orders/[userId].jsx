    ...
    <SafeAreaView style={styles.container} edges={["top"]}>
      ...
      <View style={styles.header}>
        ...
        <View style={{ flex: 1, marginLeft: 14 }}>
          ...
          <Text style={styles.headerSub}>{orders.length} order{orders.length !== 1 ? "s" : ""} shown</Text>
        </View>
        <TouchableOpacity onPress={() => load(true)}>
          <Ionicons name="refresh-outline" size={20} color={TEXT_MID} />
        </TouchableOpacity>
      </View>
      ...
      >
        {RANGE_OPTIONS.map((r) => (
          <TouchableOpacity
            key={r}
            onPress={() => setRange(r)}
            style={[styles.rangePill, range === r && { backgroundColor: TEAL, borderColor: TEAL }]}
          >
...
const styles = StyleSheet.create({
  ...
  headerTitle: { fontSize: 17, fontWeight: "800", color: TEXT_DARK },
  headerSub:   { fontSize: 11, color: TEXT_SOFT, marginTop: 1 },

  rangePills: { paddingHorizontal: 16, paddingTop: 12, paddingBottom: 8, gap: 8 },
  rangePill: {
    paddingHorizontal: 16, paddingVertical: 7,
    borderRadius: 20, borderWidth: 1,
    borderColor: "#e2e8f0", backgroundColor: CARD,
  },
