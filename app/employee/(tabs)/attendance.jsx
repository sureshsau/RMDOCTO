  ...
  const fetchAttendanceLogs = async (page = 1, isRefresh = false) => {
    try {
      ...
      const res = await api.get("/attendance/log/me", {
        params: { from, to, page, limit: LIMIT },
      });
      ...
      Toast.show({
        ...
        text2:
          err.response?.data?.message || "Could not fetch attendance logs",
      });
    } finally {
      setLoading(false);
      setRefreshing(false);
...
const LogItem = ({ log }) => {
  ...
  return (
    <View style={styles.logCard}>
      ...
      <View style={styles.logLeft}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={styles.timeRow}>
            <Ionicons name="log-in-outline" size={14} color="#14b8a6" />
            ...
            {log.lateByMinutes > 0 && (
              <View style={styles.extraBadge}>
                <Ionicons
                  ...
                  color="#b45309"
                />
                <Text style={styles.extraText}>Late {log.lateByMinutes}m</Text>
              </View>
            )}
      ...
      {/* STATUS BADGE */}
      <View style={[styles.statusBadgeNew, { backgroundColor: status.bg }]}>
        <Ionicons name={status.icon} size={14} color={status.text} />
        <Text style={[styles.statusTextNew, { color: status.text }]}>
          {status.label}
        </Text>
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  dateMonth: { fontSize: 10, color: "#64748b", fontWeight: "600" },

  logTitle: { fontWeight: "600", fontSize: 14, color: "#0f172a" },
  timeRow: {
    flexDirection: "row",
    alignItems: "center",
    marginTop: 6,
  },
  timeText: { fontSize: 12, color: "#475569", marginLeft: 4 },

  extraRow: { flexDirection: "row", marginTop: 8, gap: 8 },
  extraBadge: {
    flexDirection: "row",
    alignItems: "center",
  ...
  extraText: { fontSize: 10, color: "#b45309", marginLeft: 4, fontWeight: "600" },
  extraTextGreen: { fontSize: 10, color: "#15803d", marginLeft: 4, fontWeight: "600" },
  ...
  statusTextNew: { fontSize: 11, fontWeight: "700", marginLeft: 4 },
